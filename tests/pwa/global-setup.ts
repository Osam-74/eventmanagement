/**
 * E2E global setup:
 *  1. production build (unless BUILD_E2E=skip)
 *  2. generate a throwaway service-account key (never a real secret; the
 *     Firestore emulator ignores credentials — this only lets the server boot)
 *  3. spawn `next start` on port 3111 with emulator + test-secret env
 *  4. seed one active event + one active usher (PIN 135790) in the emulator
 * Test files read BASE_URL / usher credentials from process.env (set here).
 */
...
export default async function setup() {
  ...
  for (let i = 0; i < 60; i++) {
    try {
      const r = await fetch(`${base}/api/usher/events`);
      if (r.ok) {
        ready = true;
        break;
      }
  ...
  // Seed event + usher through the same emulator the server talks to.
